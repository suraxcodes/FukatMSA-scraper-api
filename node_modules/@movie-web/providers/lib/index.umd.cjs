(function(global2, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require("iso-639-1"), require("cheerio"), require("crypto-js"), require("unpacker"), require("nanoid"), require("form-data"), require("cookie"), require("set-cookie-parser"), require("abort-controller")) : typeof define === "function" && define.amd ? define(["exports", "iso-639-1", "cheerio", "crypto-js", "unpacker", "nanoid", "form-data", "cookie", "set-cookie-parser", "abort-controller"], factory) : (global2 = typeof globalThis !== "undefined" ? globalThis : global2 || self, factory(global2.index = {}, global2["iso-639-1"], global2.cheerio, global2["crypto-js"], global2.unpacker, global2.nanoid, global2["form-data"], global2.cookie, global2["set-cookie-parser"], global2["abort-controller"]));
})(this, function(exports2, ISO6391, cheerio, CryptoJS, unpacker, nanoid, FormData, cookie, setCookieParser, AbortController) {
  "use strict";
  ...
  async function comboScraper$g(ctx) {
    ...
    const res = await ctx.fetcher(`${baseUrl$b}/status`, { headers: headers$3 });
    if (((_a = res.providers) == null ? void 0 : _a.length) === 0) {
      throw new NotFoundError("No providers available");
    }
    const embeds = res.providers.map((provider) => {
  ...
  async function getDecryptedId(ctx) {
    ...
    const decryptedId = decrypt$2(stream.id);
    ...
    return makeEmbed({
      ...
      async scrape(ctx) {
        ...
        try {
          ...
          const search2 = await ctx.fetcher.full(searchUrl, { headers: headers$1 });
          if (search2.statusCode === 429) {
            throw new Error("Rate limited");
          } else if (search2.statusCode !== 200) {
            throw new NotFoundError("Failed to search");
  ...
  async function comboScraper$f(ctx) {
    ...
    let url = `http://127.0.0.1:6969/movie/${ctx.media.tmdbId}`;
    if (ctx.media.type === "show") {
      url = `http://127.0.0.1:6969/tv/${ctx.media.tmdbId}/${ctx.media.season.number}/${ctx.media.episode.number}`;
    }
    ...
    try {
      ...
      if (mediaInfo == null ? void 0 : mediaInfo.success) {
        ...
        if (!playlist || !Array.isArray(playlist)) {
          return { success: false, error: "Playlist not found or invalid" };
        }
...
%3A%20%22https%3A%2F%2Fcloseload.top%22%7D`;
  ...
  const turbovidScraper = makeEmbed({
    ...
    async scrape(ctx) {
      ...
      ctx.progress(90);
      const playlist = decrypt$1(data2, juiceKey);
  ...
  const keyFromProps = "4eeebc11877e087d8d3e8244cebdaae5a95579651583c43e548208a600cb705a";
  async function decrypt(ciphertext, key2) {
    try {
      const decrypted = CryptoJS.AES.decrypt(ciphertext, key2);
      return decrypted.toString(CryptoJS.enc.Utf8);
    } catch (error) {
      return null;
    }
  }
  const streamboxBase$1 = "https://vidjoy.pro/embed/api/fastfetch2";
  async function comboScraper$1(ctx) {
    var _a, _b;
    ctx.progress(10);
    const apiRes = await ctx.proxiedFetcher(
      ctx.media.type === "movie" ? `${streamboxBase$1}/${ctx.media.tmdbId}?sr=0` : `${streamboxBase$1}/${ctx.media.tmdbId}/${ctx.media.season.number}/${ctx.media.episode.number}?sr=0`
    );
    ctx.progress(50);
    if (!apiRes) {
      throw new NotFoundError("Failed to fetch StreamBox data");
    }
    let data2 = await apiRes;
    const decrypted = await decrypt(data2, keyFromProps);
    if (!decrypted) {
      throw new NotFoundError("Failed to decrypt StreamBox data");
    }
    const parsed = JSON.parse(decrypted);
    data2 = parsed;
    const streams = {};
    data2.url.forEach((stream) => {
      streams[stream.resulation] = stream.link;
    });
    ctx.progress(80);
    const captions = data2.tracks.map((track) => ({
      id: track.lang,
      url: track.url,
      language: track.code,
      type: "srt"
    }));
    if (data2.provider === "MovieBox") {
      return {
        embeds: [],
        stream: [
          {
            ...
            flags: [flags.CORS_ALLOWED],
            preferredHeaders: {
              Referer: (_a = data2.headers) == null ? void 0 : _a.Referer
            }
          }
        ]
      };
    }
    const hlsStream = data2.url.find((stream) => stream.type === "hls") || data2.url[0];
    ctx.progress(90);
    return {
      embeds: [],
      stream: [
        {
          ...
          flags: [flags.CORS_ALLOWED],
          preferredHeaders: {
            Referer: (_b = data2.headers) == null ? void 0 : _b.Referer
          }
        }
      ]
    };
  ...
  const streamboxBase = "https://vidzee.wtf";
  async function comboScraper(ctx) {
    ctx.progress(10);
    const apiRes = await ctx.proxiedFetcher(
      ctx.media.type === "movie" ? `${streamboxBase}/movie/movie.php?id=${ctx.media.tmdbId}` : `${streamboxBase}/tv/tv.php?id=${ctx.media.tmdbId}&season=${ctx.media.season.number}&episode=${ctx.media.episode.number}`
    );
    ctx.progress(50);
    if (!apiRes) {
      throw new NotFoundError("Failed to fetch StreamBox data");
    }
    const data2 = await apiRes;
    const streamRegex = /qualityOptions.?=.?(\[{.*?}\])/gm;
    const streamsData = streamRegex.exec(data2);
    if (!streamsData) {
      throw new NotFoundError("Failed to find StreamBox data");
    }
    if (streamsData.length < 2) {
      throw new NotFoundError("Failed to find StreamBox data");
    }
    const streamsJson = JSON.parse(streamsData[1]);
    const streams = {};
    streamsJson.forEach((stream) => {
      const quality = stream.html.replace("p", "");
      streams[quality] = stream.url;
    });
    ctx.progress(80);
    let captions = [];
    const subtitleRegex = /subtitleOptions.?=.?(\[{.*?}\])/gm;
    const subtitleData = subtitleRegex.exec(data2);
    let subtitlesJson = [];
    if (subtitleData) {
      if (subtitleData.length >= 2) {
        subtitlesJson = JSON.parse(subtitleData[1]);
        captions = subtitlesJson.map((track) => ({
          id: track.html,
          url: track.url,
          language: track.html,
          type: "srt",
  ...
  const stfuScraper = makeSourcerer({
    id: "stfu",
    name: "stfu",
    rank: 142,
    disabled: true,
